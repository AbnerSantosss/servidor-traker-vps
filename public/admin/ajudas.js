...
const AJUDAS = {
  ...
  'dominio-first-party': 'Subdomínio próprio do cliente (ex.: t.loja.com.br) apontado para este servidor, para que os cookies de rastreamento sejam tratados como first-party pelo navegador. Reduziria bastante o bloqueio por ad-blockers e por proteções de navegador contra cookies de terceiros — mas NESTE servidor o recurso não está disponível: publicado por túnel da Cloudflare, ele não tem IP público para o seu DNS apontar nem consegue emitir certificado para um domínio de fora da conta. A coleta continua pelos endereços deste servidor.',
  // A auditoria de 29/08/2026 pegou este texto prometendo emissão automática de certificado
  // (ver docs/17, §5). Prometer o que não sai é pior do que não ter o recurso: o operador fica
  // esperando um estágio que nunca acende, em vez de tocar a operação pelo caminho que serve.
  'dominio-indisponivel': 'O modelo "domínio do cliente" (ele aponta t.loja.com.br para cá e o servidor emite o certificado sozinho) foi desenhado para o deploy com IP público. Aqui o serviço é publicado por um Cloudflare Tunnel: não há endereço público para o DNS do cliente apontar, e a Cloudflare só emite certificado para domínio que esteja na conta dela — servir domínio de terceiro exigiria o Cloudflare for SaaS, produto pago à parte. Nada disso bloqueia a operação: as rotas /e/ e /c/ no domínio deste servidor continuam intactas, e é por elas que os eventos entram hoje.',
  'cname-vs-a': 'Duas formas de apontar o DNS para este servidor: CNAME aponta para um nome de host (mais simples de manter se o IP mudar), Registro A aponta direto para um IP fixo. Neste deploy nenhuma das duas conclui — o registro pode até resolver, mas o HTTPS do seu subdomínio não sobe por falta de certificado. A escolha só volta a importar se o serviço sair de trás do túnel.',
  ...
  'propagacao-dns': 'Tempo que os servidores de DNS do mundo todo levam para "aprender" o registro novo que você acabou de criar. Costuma levar minutos, mas pode chegar a algumas horas dependendo do provedor — clicar em Verificar de novo depois de um tempo não tem custo nenhum. Só não espere que a propagação destrave a barra de progresso: mesmo com o DNS perfeito, o domínio para em "DNS ok" (2 de 4) neste deploy.',
  'certificado-tls': 'O cadeado do navegador (HTTPS) para o seu subdomínio. No deploy com IP público este servidor emite o certificado sozinho, de graça, na primeira visita ao domínio já apontado. Aqui, não: atrás do Cloudflare Tunnel quem termina o HTTPS é a Cloudflare, e ela só emite certificado para domínio que esteja na conta dela — por isso este campo fica em "ainda não emitido" indefinidamente para domínio próprio. O HTTPS do endereço deste servidor, esse sim, funciona normalmente.',
  ...
  'notif-tipo-dominio_problema': 'Avisa quando um domínio ativo do painel para de resolver no DNS ou o certificado TLS falha ao emitir ou renovar. Neste deploy nenhum domínio próprio chega a ficar ativo (ver o aviso na aba Domínios), então este alerta fica sem uso — ele só volta a valer se o serviço sair de trás do túnel da Cloudflare.',
